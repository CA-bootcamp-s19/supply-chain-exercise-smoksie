...
/* Create a modifier that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, 'Check contract owner.');
    _;
  }

  modifier paidEnough(uint _price) {
    require(msg.value >= _price, 'Check if sufficient funds are available.');
    _;
  }
  ...
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped, 'Item shipped.');
    _;
  }

  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, 'Item received.');
    _;
  }
    ...
    checkValue(sku)
  {
    ...
    // set buyer as the person who called the transaction
